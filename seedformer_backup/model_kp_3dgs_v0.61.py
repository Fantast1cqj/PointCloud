...
Date: 2024.11.19
version: 0.61
note: 相对 0.60 在 loss 中增加 fps, 均值和 fps 后的点进行 CD   下一步改进: 分开预测均值、方差、四元数; 加一个loss, 所有点和gt的; decoder时候降采样一次加一次全局特征, cat两次; 用 so3 预测旋转矩阵
problem: 还是无法先训 means 再一起训
==============================================================
'''
import torch
...
class KP_3DGS(nn.Module):
    def __init__(self,k = 64):
        ...
        self.ge_point3 = mlp(256,self.point_number*10)
    ...
    def forward(self,partial):
        ...
        kp_gs = self.ge_point3(kp_feat)  # (B, N, 64*10)   mean_x mean_y mean_z σ1 σ2 σ3 q0 q1 q2 q3 q4
        # pp = self.pp_point3(kp_feat)
        kp_gs = torch.mean(kp_gs,dim = 1)  # (B, 64*10)
        kp_gs = kp_gs.reshape((-1, self.point_number, 10)) # (B, 64, 10)   mean_x mean_y mean_z σ1 σ2 σ3 q0 q1 q2 q3 q4
        
        B, N, _ = kp_gs.shape
        means = kp_gs[..., :3]  # (B, 64, 3) mean_x mean_y mean_z
        var_element = kp_gs[:, :, 3:6]   # (B, 64, 3) σ1 σ2 σ3
        
        epsilon = 1e-5
        var_element = torch.abs(var_element) + epsilon
        
        q = kp_gs[:, :, 6:]  # (B, 64, 4)  q0 q1 q2 q3 q4
