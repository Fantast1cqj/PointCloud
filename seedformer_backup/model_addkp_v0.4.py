...
Date: 2024-11-22
version: 0.4
note: seedformer 输入加 128 个 keypoint(完整点云)   优化 SeedGenerator 中 FeatureExtractor 部分(没训)
==============================================================
'''
...
class FeatureExtractor(nn.Module):
    def __init__(self, out_dim=1024, n_knn=20):
        ...
        self.sa_module_1 = PointNet_SA_Module_KNN(512, 16, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_2 = PointNet_SA_Module_KNN(128, 16, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_3 = PointNet_SA_Module_KNN(128, 8, 256, [256, 256], group_all=False, if_bn=False, if_idx=True)
        ...
        self.sa_kp_2 = PointNet_SA_Module_KNN(128, 4, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        ...
        self.transformer_1 = vTransformer(128, dim=64, n_knn=n_knn)
        self.transformer_2 = vTransformer(256, dim=64, n_knn=n_knn)
        self.transformer_3 = vTransformer(256, dim=64, n_knn=8)
        self.kp_transformer_1 = vTransformer(128, dim=64, n_knn=8)
        self.kp_transformer_2 = vTransformer(256, dim=64, n_knn=4)
    ...
    def forward(self, partial_cloud, kp):
        ...
        # layer 1
        l1_xyz, l1_points, idxl1 = self.sa_module_1(l0_xyz, l0_points)  # (B, 3, 512), (B, 128, 512)  l1_xyz：512个点xyz坐标 l1_points：512个点，128维特征
        l1_points = self.transformer_1(l1_points, l1_xyz)   # l1_points用transformer增强特征
        # kp1 feat
        ...
        # layer 2
        mix1_xyz, mix1_points, idmix1 = self.sa_module_2(cat1_xyz, cat1_point) # (B, 3, 128) (B, 256, 128)
        mix1_points = self.transformer_2(mix1_points, mix1_xyz)  # (B, 256, 128)
        # kp2 feat
        kp2_xyz, kp2_points, pk2_id = self.sa_kp_2(kp1_xyz, kp1_points)  # (B, 3, 128)  (B, 256, 128)
        kp2_points = self.kp_transformer_2(kp2_points, kp2_xyz)          # (B, 256, 128)
        # cat kp2 & layer 2
        cat2_xyz = torch.cat((mix1_xyz, kp2_xyz), dim=2) # (B, 3, 128+128)
        cat2_point = torch.cat((mix1_points, kp2_points), dim=2) # (B, 256, 256)
        # layer 3
        mix2_xyz, mix2_points, idmix2 = self.sa_module_3(cat2_xyz, cat2_point) # (B, 3, 128) (B, 256, 128)
        mix2_points = self.transformer_3(mix2_points, mix2_xyz)  # (B, 256, 128)
        # layer 4
        l3_xyz, feat_all = self.sa_module_4(mix2_xyz, mix2_points)  # (B, 3, 1), (B, out_dim, 1)   128个点变成1个点，特征变成一个out_dim维的全局特征
        

        return feat_all, mix2_xyz, mix2_points
