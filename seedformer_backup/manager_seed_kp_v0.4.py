...
SeedFormer: Point Cloud Completion + key poins cross attention
...
class Manager:
    ...
    def test(self, cfg, model, kp_model, test_data_loader, outdir, mode=None):

        if self.dataset == 'ShapeNet':
            self.test_pcn(cfg, model,kp_model ,test_data_loader, outdir)
        elif self.dataset == 'ShapeNet55':
            self.test_shapenet55(cfg, model, kp_model, test_data_loader, outdir, mode)
        else:
            raise ValueError('No testing method implemented for this dataset: {:s}'.format(self.dataset))

    def test_pcn(self, cfg, model=None, kp_model=None,test_data_loader=None, outdir=None):
        """
